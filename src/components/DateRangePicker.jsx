...
function fmtDate(value, locale) {
  ...
  return {
    ...
    full: d.toLocaleDateString(locale, { weekday: 'short', day: 'numeric', month: 'short', year: 'numeric', ...tz }),
  };
}
...
function getMonthOffset(value, baseYear, baseMonth) {
  ...
  const d = new Date(`${value}T00:00:00+08:00`);
  return Math.max(0, (d.getFullYear() - baseYear) * 12 + (d.getMonth() - baseMonth));
}

function MonthGrid({ year, month, startDate, endDate, hoverDate, today, onDayClick, onDayHover, isIndonesian }) {
  ...
  return (
    <div className="cpl-cal-month">
      ...
      <div className="cpl-cal-weekdays">
        {dayNames.map((day) => <span key={day} className="cpl-cal-weekday">{day}</span>)}
      </div>
      <div className="cpl-cal-days">
        {cells.map((day, index) => {
          if (!day) {
            return <span key={`empty-${index}`} className="cpl-cal-day-wrapper" />;
          }
...
export function DateRangePicker({
  ...
  const updatePos = useCallback(() => {
    ...
    const top = below >= 500 || below >= window.innerHeight * 0.45 ? rect.bottom + 6 : rect.top - 506;
    setPopupStyle({ position: 'fixed', top: top < pad ? rect.bottom + 6 : top, left, width, zIndex: 9999 });
  }, [isMobile]);
  ...
  const handleDayClick = useCallback((ds) => {
    ...
    setHoverDate(null);
    closePicker();
  }, [startDate, endDate, baseY, baseM, onStartDateChange, onEndDateChange, closePicker]);
  ...
  const handleBackdropPointer = useCallback((event, shouldClose = false) => {
    event.preventDefault();
    event.stopPropagation();
  ...
  const calendar = open ? createPortal(
    <>
      <div
        className={isMobile ? 'cpl-cal-backdrop' : 'cpl-cal-backdrop cpl-cal-backdrop-desktop'}
        data-cpl-datepicker=""
      ...
      />
      {isMobile ? (
        <div className="cpl-cal-sheet" data-cpl-datepicker="" ref={popupRef} onPointerDown={handlePopupPointer} onPointerUp={handlePopupPointer} onTouchStart={handlePopupPointer} onTouchEnd={handlePopupPointer} onClick={handlePopupPointer}>
          <div className="cpl-cal-sheet-handle" aria-hidden="true" />
          {renderCalendar()}
        </div>
      ) : (
        <div className="cpl-cal-popup" data-cpl-datepicker="" style={popupStyle} ref={popupRef} onPointerDown={handlePopupPointer} onPointerUp={handlePopupPointer} onTouchStart={handlePopupPointer} onTouchEnd={handlePopupPointer} onClick={handlePopupPointer}>
          {renderCalendar()}
        </div>
      )}
    </>,
    ...
    return (
      <>
        <div className="cpl-cal-popup-header">
          <div className="cpl-cal-header-info">
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              {startDate && endDate && (
                <button type="button" className="cpl-cal-clear-btn" onClick={handleClear}>
                  <X size={12} /> {isIndonesian ? 'Reset' : 'Clear'}
                </button>
              )}
              {isMobile && (
                <button type="button" className="cpl-cal-close-btn" onClick={closePicker} aria-label="Close">
                  <X size={16} />
                </button>
              )}
            </div>
          ...
          <div className="cpl-cal-nav">
            ...
            <span className="cpl-cal-nav-label">
              {monthNames[lM]} {lY}{!isMobile && <> – {monthNames[rM]} {rY}</>}
            </span>
            <button
              type="button"
              className="cpl-cal-nav-btn"
        ...
        <div className={isMobile ? 'cpl-cal-months-mobile' : 'cpl-cal-months'}>
          <MonthGrid year={lY} month={lM} startDate={startDate} endDate={endDate} hoverDate={hoverDate} today={today} onDayClick={handleDayClick} onDayHover={setHoverDate} isIndonesian={isIndonesian} />
          {!isMobile && <><div className="cpl-cal-month-sep" /><MonthGrid year={rY} month={rM} startDate={startDate} endDate={endDate} hoverDate={hoverDate} today={today} onDayClick={handleDayClick} onDayHover={setHoverDate} isIndonesian={isIndonesian} /></>}
          {isMobile && <MonthGrid year={rY} month={rM} startDate={startDate} endDate={endDate} hoverDate={hoverDate} today={today} onDayClick={handleDayClick} onDayHover={setHoverDate} isIndonesian={isIndonesian} />}
        </div>
