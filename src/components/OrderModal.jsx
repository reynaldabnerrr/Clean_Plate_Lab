...
const TIER_OPTIONS = proteinTiers.map(({ protein, prices }) => ({
  tier: protein,
  prices,
}));
...
export function OrderModal({
  ...
  const [singleMealReadyTime, setSingleMealReadyTime] = useState(
    storedDraft?.singleMealReadyTime ?? "12:00",
  );
  ...
  const addonsPerBox = selectedAddons.reduce(
  ...
  const orderCopy = isIndonesian
    ? {
        ...
        scheduleNote:
          "Makan siang siap pukul 12.00 dan makan malam siap pukul 18.00.",
        lunch: "Makan siang",
        dinner: "Makan malam",
        ready: "siap",
        ...
        selfArrangedNote:
          "Pesanan dapat diambil sendiri atau melalui kurir yang Anda atur dari alamat Clean Plate Lab berikut:",
        ...
        courierNote:
          "Biaya kurir belum termasuk dan dikonfirmasi melalui WhatsApp.",
      }
    : {
        ...
        selfArrangedNote:
          "Collect the order yourself or arrange a courier to collect it from the following Clean Plate Lab address:",
        ...
        courierNote:
          "Courier fees are excluded and confirmed through WhatsApp.",
      };
  ...
  const selectedFulfillmentLabel =
    fulfillmentOptions.find((option) => option.id === fulfillment)?.label ||
    fulfillment;
  const selectedAddonNames = selectedAddons.map((addon) =>
    isIndonesian ? addon.nameID : addon.name,
  );
  const scheduleLabel =
    mealsPerDay === 1 ? orderCopy.singleSchedule : orderCopy.dualSchedule;
  const dayCountLabel = isIndonesian
    ? `${totalDays} hari`
    : `${totalDays} ${totalDays === 1 ? "day" : "days"}`;
  const boxCountLabel = isIndonesian
    ? `${totalBoxes} box`
  ...
  useEffect(() => {
    if (
      isOpen &&
      hasExplicitInitialValues &&
      TIER_OPTIONS.some((option) => option.tier === initialProteinTier)
    ) {
  ...
  const validateForm = () => {
    ...
    if (requiresDeliveryAddress && !address.trim()) {
      newErrors.address =
        t("orderAddressReqError") || "Alamat pengiriman lengkap wajib diisi";
    }
    ...
    if (
      ...
      newErrors.mapsUrl =
        t("orderMapsInvalidError") || "Masukkan tautan Google Maps yang valid";
    }
    ...
    <Dialog open={isOpen} onOpenChange={handleOpenChange}>
      <DialogContent
        ...
        onPointerDownOutside={(event) => {
          if (
            datePickerOpenRef.current ||
            event.target?.closest?.("[data-cpl-datepicker]")
          ) {
            event.preventDefault();
          }
        }}
        ...
        {!submitted ? (
          <form noValidate onSubmit={handleSubmit} className="min-w-0">
            <DialogHeader className="relative mx-3 mt-3 overflow-hidden rounded-[22px] border-0 bg-[#1E1E1E] py-5 pl-5 pr-16 text-left shadow-[0_14px_32px_rgba(30,30,30,0.18)] sm:mx-4 sm:mt-4 sm:rounded-[26px] sm:py-6 sm:pl-7 sm:pr-20">
              <div
                className="absolute inset-y-0 right-0 hidden w-40 border-l border-white/10 sm:block"
                aria-hidden="true"
              >
                <div className="grid h-full grid-cols-5 opacity-20">
                  {Array.from({ length: 15 }).map((_, index) => (
                    <span
                      key={index}
                      className="border-b border-r border-white/30"
                    />
                  ))}
                </div>
              </div>
              ...
              <div className="relative flex items-center gap-2.5">
                <CplLogoImage
                  size={42}
                  className="border-white/40 shadow-none"
                />
            ...
            <div className="grid min-w-0 gap-5 p-4 sm:p-7 md:grid-cols-2 md:gap-7">
              <div className="min-w-0 space-y-5">
                  ...
                  <div className="grid grid-cols-1 gap-3 lg:grid-cols-2">
                    <div className="min-w-0 space-y-1.5">
                      <label
                        ...
                        className="block font-display text-[10px] font-bold uppercase text-[#4D4D4D]"
                      >
                        {t("orderName")}
                      </label>
                      <div className="relative">
                        <User
                          size={15}
                          className={`pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 transition-colors ${errors.name ? "text-[#C93B2B]" : "text-[#6B7860]"}`}
                        />
                        <Input
                          id={`${fieldId}-name`}
                          type="text"
                        ...
                        {errors.name && (
                          ...
                          >
                            <AlertCircle
                              size={13}
                              className="shrink-0 text-[#C93B2B]"
                            />
                    ...
                    <div className="min-w-0 space-y-1.5">
                      ...
                      <div className="relative">
                        <Phone
                          size={15}
                          className={`pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 transition-colors ${errors.phone ? "text-[#C93B2B]" : "text-[#6B7860]"}`}
                        />
                        <Input
                          id={`${fieldId}-phone`}
                          type="tel"
                          ...
                          placeholder={
                            t("orderPhonePlaceholder") || "0812 3456 7890"
                          }
                          className={`h-11 w-full max-w-full min-w-0 rounded-lg pl-9 pr-3 font-sans text-xs font-semibold transition-all duration-200 ${
                            errors.phone
                              ? "border-2 border-[#C93B2B] bg-[#FDF5F5] text-[#1E1E1E] focus-visible:ring-2 focus-visible:ring-[#C93B2B]/20"
                  ...
                  <div className="mt-3 flex min-w-0 items-start gap-3 rounded-lg border border-[#8D9B7D]/50 bg-[#E1ECD3] p-3.5">
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="font-display text-[11px] font-extrabold uppercase text-[#3A4A30]">
                        {mealsPerDay}{" "}
                        {isIndonesian
                          ? `box segar setiap hari`
                          : `fresh ${mealsPerDay === 1 ? "box" : "boxes"} every day`}
                      </p>
                    ...
                    <div className="hidden shrink-0 flex-col gap-1 font-mono text-[8px] font-bold uppercase text-[#6B7860] sm:flex">
                      <span className="inline-flex items-center gap-1">
                        <Clock3 size={10} /> {mealsPerDay}x / day
                      </span>
                      <span className="inline-flex items-center gap-1">
                        <UtensilsCrossed size={10} /> {mealsPerDay}{" "}
                        {mealsPerDay === 1 ? "box" : "boxes"} / day
                      </span>
                    </div>
                  </div>
                </section>
                ...
                <section aria-labelledby={`${fieldId}-addons-title`}>
                    ...
                    {addons.map((addon) => {
                      ...
                      return (
                        <button
                          ...
                          className={`flex min-h-16 items-center justify-between gap-3 rounded-lg border-2 p-3 text-left transition-colors ${selected ? "border-[#1E1E1E] bg-[#1E1E1E] text-white" : "border-[#1E1E1E]/15 bg-white hover:border-[#D1954E]"}`}
                        >
                          <span className="min-w-0">
                            <strong className="block font-display text-[10px] font-extrabold uppercase leading-tight">
                              {isIndonesian ? addon.nameID : addon.name}
                            </strong>
                            <span
                              className={`mt-1 block font-mono text-[9px] font-bold ${selected ? "text-[#C8D8B8]" : "text-[#6B7860]"}`}
                            >
                              + Rp {addon.price.toLocaleString("id-ID")} / box
                            </span>
                          </span>
                          {selected ? (
                            <Check
                              size={16}
                              className="shrink-0 text-[#C8D8B8]"
                            />
                          ) : null}
                        </button>
                      );
                  ...
                  <p className="mt-2 text-[9px] leading-4 text-[#6B7860]">
                    {orderCopy.addonsHelp}
                  </p>
                </section>
              </div>

              <section
                aria-labelledby={`${fieldId}-delivery-title`}
                className="min-w-0"
              >
                <div className="mb-3 flex items-center gap-2.5">
                  <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-[#1E1E1E] font-mono text-[10px] font-bold text-white">
                ...
                <div className="mt-5">
                  <div className="mb-2 flex items-center gap-2.5">
                    ...
                    <p className="font-display text-[10px] font-extrabold uppercase">
                      {orderCopy.servingsPerDay}
                    </p>
                    <span className="h-px min-w-[8px] flex-1 bg-[#1E1E1E]/15" />
                  </div>
                  <div
                    ...
                    role="radiogroup"
                    aria-label={orderCopy.servingsPerDay}
                  >
                    <button
                      type="button"
                      role="radio"
                      aria-checked={mealsPerDay === 1}
                      onClick={() => setMealsPerDay(1)}
                      className={`min-h-11 rounded-lg border-2 px-3 font-display text-[10px] font-extrabold uppercase transition-all duration-200 hover:-translate-y-0.5 hover:border-[#D1954E] hover:shadow-sm ${mealsPerDay === 1 ? "border-[#1E1E1E] bg-[#1E1E1E] text-white" : "border-[#1E1E1E]/20 bg-white"}`}
                    >
                      {orderCopy.oneServing}
                    </button>
                    <button
                      type="button"
                      role="radio"
                      aria-checked={mealsPerDay === 2}
                      onClick={() => setMealsPerDay(2)}
                      className={`min-h-11 rounded-lg border-2 px-3 font-display text-[10px] font-extrabold uppercase transition-all duration-200 hover:-translate-y-0.5 hover:border-[#D1954E] hover:shadow-sm ${mealsPerDay === 2 ? "border-[#1E1E1E] bg-[#1E1E1E] text-white" : "border-[#1E1E1E]/20 bg-white"}`}
                    >
                      {orderCopy.twoServings}
                    </button>
                  </div>
                  {mealsPerDay === 2 ? (
                    <p className="mt-2 rounded-lg border border-[#8D9B7D]/40 bg-[#E1ECD3] px-3 py-2 text-[10px] font-semibold leading-4 text-[#6B7860]">
                      {orderCopy.sameMenu}
                    </p>
                  ) : null}
                </div>
                ...
                <div className="mt-4 rounded-lg border border-[#1E1E1E]/20 bg-white p-3">
                  <p className="flex items-center gap-1.5 font-display text-[10px] font-bold uppercase text-[#4D4D4D]">
                    <Clock3 size={13} className="text-[#6B7860]" />
                    {scheduleLabel}
                  </p>
                  {mealsPerDay === 1 ? (
                    <div
                      className="mt-2 grid grid-cols-2 gap-2"
                      role="radiogroup"
                      aria-label={orderCopy.singleScheduleNote}
                    >
                      {[
                        ...
                        return (
                          ...
                          >
                            <span
                              className={`font-display text-[9px] font-bold uppercase ${isSelected ? "text-[#C8D8B8]" : "text-[#6B7860]"}`}
                            >
                              {label}
                            </span>
                    ...
                    <div className="mt-2 grid grid-cols-2 gap-2">
                      <div className="flex items-center justify-between rounded-md bg-[var(--cpl-cream)] px-3 py-2">
                        <span className="font-display text-[9px] font-bold uppercase text-[#6B7860]">
                          {orderCopy.lunch}
                        </span>
                        <strong className="font-mono text-xs">12.00</strong>
                      </div>
                      <div className="flex items-center justify-between rounded-md bg-[var(--cpl-cream)] px-3 py-2">
                        <span className="font-display text-[9px] font-bold uppercase text-[#6B7860]">
                          {orderCopy.dinner}
                        </span>
                        <strong className="font-mono text-xs">18.00</strong>
                      </div>
                    </div>
                  )}
                  <p className="mt-2 text-[9px] leading-4 text-[#6B7860]">
                    {mealsPerDay === 1
                      ? orderCopy.singleScheduleNote
                      : orderCopy.scheduleNote}
                  </p>
                </div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {fulfillmentOptions.map((option) => (
                      <button
                        ...
                        className={`min-h-11 rounded-lg border-2 px-2 font-display text-[9px] font-extrabold uppercase leading-tight transition-all duration-200 hover:-translate-y-0.5 hover:border-[#D1954E] hover:shadow-sm ${fulfillment === option.id ? "border-[#1E1E1E] bg-[#1E1E1E] text-white" : "border-[#1E1E1E]/20 bg-white"}`}
                      >
                        {option.label}
                      </button>
                    ))}
                ...
                {requiresDeliveryAddress ? (
                  <div className="mt-4 space-y-3">
                    <div className="space-y-1.5">
                      ...
                      <div className="relative">
                        <MapPin
                          size={15}
                          className={`pointer-events-none absolute left-3 top-3 transition-colors ${errors.address ? "text-[#C93B2B]" : "text-[#6B7860]"}`}
                        />
                        <textarea
                          id={`${fieldId}-address`}
                          rows={2}
                    ...
                    <div className="space-y-1.5">
                      ...
                      <div className="relative">
                        <Link2
                          size={15}
                          className="pointer-events-none absolute left-3 top-3 text-[#6B7860]"
                        />
                        <Input
                          id={`${fieldId}-maps-url`}
                          type="url"
                          ...
                          onChange={(event) => {
                            ...
                            if (
                              errors.mapsUrl &&
                              (!value.trim() || isValidGoogleMapsUrl(value))
                            ) {
                              setErrors((current) => ({
                                ...current,
                                mapsUrl: undefined,
                      ...
                      {errors.mapsUrl ? (
                        <p
                          id={`${fieldId}-maps-help`}
                          role="alert"
                          className="flex items-center gap-1.5 text-[10px] font-bold leading-4 text-[#8A1F17]"
                        >
                          <AlertCircle
                            size={12}
                            className="shrink-0 text-[#C93B2B]"
                        ...
                        <p
                          id={`${fieldId}-maps-help`}
                          className="text-[9px] leading-4 text-[#6B7860]"
                        >
                          {t("orderMapsHelp")}
                        </p>
                      )}
                  ...
                  <div className="mt-3 rounded-lg border border-[#8D9B7D]/40 bg-[#E1ECD3] px-3 py-2 text-[9px] font-semibold leading-4 text-[#6B7860]">
                    ...
                    <div className="mt-1.5 flex items-start gap-1.5 font-bold text-[#3A4A30]">
                      ...
                      <a
                        ...
                        className="hover:text-[#D1954E] hover:underline"
                      >
                        {centralKitchenAddress}
                      </a>
                    </div>
                    <div className="mt-1.5 flex items-center gap-2">
                      <button
                        ...
                        className="inline-flex items-center gap-1 rounded bg-[#1E1E1E]/10 px-2 py-1 font-mono text-[8px] font-bold text-[#3A4A30] hover:bg-[#1E1E1E]/20"
                      >
                        {copiedMapLink ? (
                          <>
                ...
                <div
                  className="mt-5 overflow-hidden rounded-2xl border-2 border-[#8D9B7D] bg-[#E1ECD3]"
                  aria-live="polite"
                >
                  <div className="flex items-center gap-3 border-b border-[#8D9B7D]/35 px-3.5 py-3 sm:px-4">
                    <p className="flex min-w-0 items-center gap-2 font-display text-[10px] font-extrabold uppercase text-[#3A4A30]">
                      <span className="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-[#1E1E1E] font-mono text-[9px] text-white">
                        07
                      </span>
                  ...
                  <div className="space-y-2.5 p-3 sm:p-3.5">
                    <div className="grid grid-cols-2 gap-2">
                      <div className="rounded-xl border border-[#8D9B7D]/25 bg-white p-3">
                        <p className="font-display text-[8px] font-bold uppercase tracking-wide text-[#6B7860]">
                          {orderCopy.weeklyRotation}
                        </p>
                        <p className="mt-1 font-display text-xs font-black leading-tight text-[#1E1E1E] sm:text-sm">
                          {selectedTier.tier}g Protein · {periodLabel}
                        </p>
                        <p className="mt-0.5 text-[9px] leading-4 text-[#6B7860]">
                          {dayCountLabel} · {boxCountLabel}
                        </p>
                      </div>
                      <div className="rounded-xl border border-[#8D9B7D]/25 bg-white p-3">
                        <p className="font-display text-[8px] font-bold uppercase tracking-wide text-[#6B7860]">
                        ...
                        <p className="mt-1 font-display text-sm font-black text-[#1E1E1E]">
                    ...
                    <div className="divide-y divide-[#1E1E1E]/10 rounded-xl border border-[#8D9B7D]/25 bg-white px-3">
                      <div className="flex items-start gap-2.5 py-2.5">
                        <CalendarDays
                          size={14}
                          className="mt-0.5 shrink-0 text-[#6B7860]"
                        />
                        <div className="min-w-0">
                          <p className="font-display text-[8px] font-bold uppercase tracking-wide text-[#6B7860]">
                            {isIndonesian
                          ...
                          <p className="mt-0.5 text-[10px] font-semibold text-[#1E1E1E]">
                            {formatOrderDate(startDate, locale)} –{" "}
                            {formatOrderDate(endDate, locale)}
                          </p>
                        </div>
                      </div>
                      <div className="flex items-start gap-2.5 py-2.5">
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="mt-0.5 text-[10px] font-bold text-[#1E1E1E]">
                      ...
                      <div className="flex items-start gap-2.5 py-2.5">
                        ...
                        <div className="min-w-0">
                          <p className="font-display text-[8px] font-bold uppercase tracking-wide text-[#6B7860]">
                            {requiresDeliveryAddress
                              ? orderCopy.deliveryDestination
                              : orderCopy.centralKitchen}
                          </p>
                          <p className="mt-0.5 break-words text-[9px] leading-4 text-[#1E1E1E]">
                            {requiresDeliveryAddress
                              ? address || "-"
                              : centralKitchenAddress}
                          </p>
                          {requiresDeliveryAddress && mapsUrl.trim() ? (
                            <p className="mt-1 flex min-w-0 items-start gap-1 text-[9px] text-[#6B7860]">
                      ...
                      <div className="flex items-start gap-2.5 py-2.5">
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="mt-0.5 text-[9px] font-semibold leading-4 text-[#1E1E1E]">
                            {mealsPerDay === 1
                              ? `${singleMealReadyTime === "12:00" ? orderCopy.lunch : orderCopy.dinner} ${orderCopy.ready} ${formatReadyTime(readyTimeMeal1)}`
                              : `${orderCopy.lunch} ${orderCopy.ready} ${formatReadyTime(readyTimeMeal1)} · ${orderCopy.dinner} ${orderCopy.ready} ${formatReadyTime(readyTimeMeal2)}`}
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="overflow-hidden rounded-xl border border-[#8D9B7D]/25 bg-white font-mono text-[9px]">
                      <div className="flex items-start justify-between gap-3 px-3 py-2.5">
                        ...
                        <strong className="whitespace-nowrap text-[#3A4A30]">
                          Rp{" "}
                          {(selectedPrice * totalBoxes).toLocaleString("id-ID")}
                        </strong>
                      </div>
                      <div className="border-t border-[#1E1E1E]/10 px-3 py-2.5">
                        <p className="font-sans text-[8px] font-bold uppercase tracking-wide text-[#6B7860]">
                          {orderCopy.addons}
                        </p>
                        {selectedAddons.length ? (
                          selectedAddons.map((addon) => (
                            ...
                            >
                              ...
                              <strong className="whitespace-nowrap text-[#3A4A30]">
                                Rp{" "}
                                {(addon.price * totalBoxes).toLocaleString(
                                  "id-ID",
                                )}
                              </strong>
                            </div>
                          ...
                          <p className="mt-1 font-sans text-[10px] font-semibold text-[#1E1E1E]">
                            {orderCopy.none}
                          </p>
                        )}
                        {selectedAddons.length ? (
                          <div className="mt-2 flex justify-between gap-3 border-t border-[#1E1E1E]/10 pt-2 font-bold text-[#3A4A30]">
                            <span>{orderCopy.addonTotal}</span>
                            <span className="whitespace-nowrap">
                              Rp{" "}
                              {(addonsPerBox * totalBoxes).toLocaleString(
                                "id-ID",
                              )}
                            </span>
                          </div>
                    ...
                    <div className="flex items-center justify-between gap-3 rounded-xl bg-[#1E1E1E] px-3.5 py-3 text-white">
                      <p className="font-display text-[9px] font-bold uppercase tracking-wide text-white/65">
                        {t("orderEstimatedTotal")}
                      </p>
                      <p className="whitespace-nowrap font-display text-xl font-black">
                        Rp {totalCost.toLocaleString("id-ID")}
                      </p>
                    </div>
                  </div>
                </div>
                <p className="mt-2 text-center text-[9px] leading-4 text-[#6B7860]">
                  {orderCopy.courierNote}
                </p>
                ...
                >
                  <span className="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-white/15 font-mono text-[9px] font-bold text-white">
                    08
                  </span>
            ...
            <div className="relative mx-auto max-w-xl text-center">
              <div className="relative mx-auto flex h-16 w-16 items-center justify-center rounded-full border-2 border-[#8D9B7D]/40 bg-[#E1ECD3] text-[#6B7860] shadow-md animate-in zoom-in-50 duration-300 sm:h-20 sm:w-20">
                <CheckCircle
                  size={38}
                  strokeWidth={2.5}
                  className="text-[#6B7860] sm:size-11"
                />
              ...
              <div className="mt-5 rounded-2xl border border-[#8D9B7D]/40 bg-[#E1ECD3] p-4 text-left shadow-xs sm:p-5">
                ...
                <div className="mt-3.5 grid grid-cols-1 gap-3 sm:grid-cols-2">
                  <div>
                    ...
                    <p className="mt-0.5 font-sans text-xs font-bold text-[#1E1E1E]">
                      {name}
                    </p>
                  </div>
                  ...
                  <div className="sm:col-span-2">
                    ...
                    <p className="mt-0.5 font-mono text-[10px] font-bold text-[#1E1E1E]">
                      {dayCountLabel} · {mealsPerDay}x/
                      {isIndonesian ? "hari" : "day"} · {boxCountLabel}
                    </p>
                  </div>
                  <div className="sm:col-span-2">
                    <p className="font-display text-[9px] font-bold uppercase tracking-wider text-[#6B7860]">
                  ...
                  <div>
                    ...
                    {requiresDeliveryAddress ? (
                      <div className="sm:col-span-2">
                        ...
                        <p className="mt-0.5 text-[10px] font-bold leading-4 text-[#1E1E1E]">
                          {address}
                        </p>
                        {mapsUrl.trim() ? (
                          <div className="mt-1 flex items-center justify-between gap-2 break-all">
                            <a
                              ...
                              target="_blank"
                              rel="noopener noreferrer"
                              className="inline-flex items-start gap-1 text-[9px] font-semibold text-[#6B7860] underline underline-offset-2"
                            >
                              <Link2 size={10} className="mt-0.5 shrink-0" />
                              {mapsUrl.trim()}
                            </a>
                            <button
                              type="button"
                              onClick={() =>
                                navigator.clipboard.writeText(mapsUrl.trim())
                              }
                              className="inline-flex items-center gap-1 rounded bg-[#1E1E1E]/10 px-2 py-1 font-mono text-[8px] font-bold text-[#6B7860] hover:bg-[#1E1E1E]/20"
                            >
                              Copy {t("orderMapsSummary")}
                            </button>
                          </div>
                        ) : null}
                      </div>
                      ...
                      <div className="mt-1">
                        ...
                        <p className="mt-0.5 break-words text-[10px] font-bold leading-4 text-[#1E1E1E]">
                          {centralKitchenAddress}
                        </p>
                        <div className="mt-1 flex items-center justify-between gap-2 break-all">
                          <a
                            ...
                            target="_blank"
                            rel="noopener noreferrer"
                            className="inline-flex items-center gap-1.5 text-[9px] font-semibold text-[#6B7860] underline underline-offset-2"
                          >
                            <Link2 size={10} />
                            {t("footerKitchenMapsLink")}
                          </a>
                          <button
                            type="button"
                            onClick={() =>
                ...
                <div className="mt-4 flex items-center justify-between rounded-xl border border-[#1E1E1E]/15 bg-white px-4 py-3 shadow-xs">
                  <span className="font-display text-xs font-bold uppercase tracking-wider text-[#6B7860]">
                    {t("orderEstimatedTotal")}
                  </span>
                  <span className="font-display text-xl font-black text-[#1E1E1E] sm:text-2xl">
                    Rp {totalCost.toLocaleString("id-ID")}
                  </span>
                </div>
              </div>
