...
export function FaqSection() {
  ...
  return (
    <section id="faq" className="scroll-mt-20 border-b border-[var(--cpl-border)] bg-[var(--cpl-white)] py-12 sm:py-24">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-6 flex flex-col gap-3 border-b-2 border-black pb-5 sm:mt-10 sm:gap-4 sm:pb-7 lg:flex-row lg:items-center lg:justify-between">
          <label className="relative block w-full max-w-lg">
            ...
            <input type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder={copy.faq.search} className="min-h-11 w-full rounded-lg border-2 border-[#1E1E1E] bg-[var(--cpl-cream)] pl-11 pr-4 text-xs outline-none focus:ring-2 focus:ring-[var(--cpl-sage)] sm:min-h-12 sm:text-sm" />
          </label>
          <div className="flex max-w-full flex-wrap gap-2" aria-label="FAQ categories">
            {categories.map((item) => <button key={item} type="button" aria-pressed={category === item} onClick={() => setCategory(item)} className={`min-h-10 whitespace-nowrap border-2 border-[#1E1E1E] px-3 text-[9px] font-bold uppercase tracking-wider sm:min-h-11 sm:px-4 sm:text-[10px] ${category === item ? 'bg-[var(--cpl-dark)] text-white' : 'bg-[var(--cpl-cream)]'}`}>{categoryLabel(item)}</button>)}
          </div>
        </div>
        <div className="mx-auto mt-5 grid max-w-4xl gap-2.5 sm:mt-8 sm:gap-4">
          {filteredFaqs.map((faq, index) => (
            <details key={faq.question} className={`${shouldLimitMobileResults && index >= 5 ? 'hidden sm:block' : ''} group border-2 border-[#1E1E1E] bg-[var(--cpl-cream)] px-4 py-3.5 shadow-[2px_2px_0_#1E1E1E] sm:px-6 sm:py-5 sm:shadow-[3px_3px_0_#1E1E1E]`}>
              ...
              <p className="mt-3 max-w-3xl border-t border-black/20 pt-3 text-xs leading-5 text-[var(--cpl-dark-muted)] sm:mt-4 sm:pt-4 sm:text-sm sm:leading-7">{isIndonesian ? faq.answerID : faq.answer}</p>
              <p className="mt-2 font-mono text-[8px] font-bold uppercase tracking-wider text-[var(--cpl-sage-dark)] sm:mt-3 sm:text-[9px]">{isIndonesian ? faq.categoryID : faq.category}</p>
            </details>
          ))}
